...
test('HTTP AI 경로의 자료·범위 분리, 사용자 근거 검증 및 자동 저장 금지',async()=>{
 ...
 await writeFile(fake,`#!${process.execPath}\nimport {appendFileSync} from 'node:fs';let input='';for await(const c of process.stdin)input+=c;appendFileSync(${JSON.stringify(capture)},JSON.stringify(input)+'\\n');console.log(JSON.stringify({structured_output:{draft:'검증 응답',facts:'',style:'검증 후보',questions:[],changes:[],evidence:[{sourceId:'assistant',quote:'AI 제안',claim:'사용자 선호'}]}}));`,{mode:0o700});
 ...
 try{
  ...
  const generated=await call('/api/ai',req);assert.equal(generated.status,200);assert.equal(generated.body.factReview.status,'blocked');assert.equal(generated.body.draft,'');
  const prompts=async()=>(await readFile(capture,'utf8')).trim().split('\n').map(JSON.parse);
  let prompt=(await prompts()).at(-2);assert.doesNotMatch((await prompts()).at(-1),/SECRET_|LOCAL_RULE/);assert.doesNotMatch(prompt,/SECRET_|LOCAL_RULE/);
  assert.equal((await call('/api/ai',{...req,company:'a'})).status,200);prompt=(await prompts()).at(-2);assert.doesNotMatch((await prompts()).at(-1),/SECRET_|LOCAL_RULE/);assert.match(prompt,/LOCAL_RULE/);assert.doesNotMatch(prompt,/SECRET_/);
  ...
  prompt=(await prompts()).at(-1);assert.doesNotMatch(prompt,/검증된 행동|LOCAL_RULE|SECRET_/);
