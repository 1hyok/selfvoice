...
export function validateState(value) {
  ...
  for (const kind of ['sources','memories','drafts']) {
    ...
    for (const row of value[kind]) {
      ...
      for(const key of ['sourceIds','styleSourceIds'])if(row[key]!==undefined&&(!Array.isArray(row[key])||!row[key].every(x=>typeof x==='string')))throw new Error('참고 자료 목록이 올바르지 않습니다.');
...
export function styleEvidence(example) {
  ...
  return {sentenceLengths:sentences.map(s=>[...s].length),sampleCount:1,sentenceCount:n,min:lengths[0],median:n%2?lengths[(n-1)/2]:(lengths[n/2-1]+lengths[n/2])/2,max:lengths[n-1],mean:Math.round(lengths.reduce((a,b)=>a+b,0)/n*10)/10};
}
export function makeContext(state,request) {
  const styleIds=request.styleSourceIds??(request.sourceIds||[]);
  const voices=state.sources.filter(s=>s.status==='reference'&&s.usage==='both'&&styleIds.includes(s.id)&&(s.example?.trim()||s.style?.trim())&&!findCorrections([s.facts,s.style,s.example].join(' '),state.corrections).length);
  ...
  return {
    ...
    sources:selected.map(s=>({id:s.id,title:s.title,facts:text(s.facts,16000),basis:text(s.basis,2000)})),
    voices:voices.map(s=>({id:s.id,styleExample:text(s.example,10000),styleRules:text(s.style,2000),styleEvidence:styleEvidence(text(s.example,10000))})),
...
말투 적용 시 사용자가 명시한 preferences와 이번 instructions를 우선한다. styleRules는 승인된 관찰을 현재 문항에 맞게 적용하며, 예문의 소재나 감정까지 모든 글의 규칙으로 확대하지 않는다. styleEvidence는 예문 한 편의 대략적인 문장 길이 관찰값이며 목표 점수나 맞춰야 할 길이가 아니다. 짧은 진술과 이유·대조를 설명하는 긴 문장이 섞여 있으면 그 변화도 살린다. 단문으로 통일하거나 문장을 늘려 평균을 맞추지 않는다. 기술 용어와 구체적인 생각을 추상적인 역량·기여 표현으로 치환하지 않는다. 내용상 이유가 없는 자조·비유·독백·다짐을 추가하지 않는다. 문항이 묻는 것에 답하는 결말을 쓰고 모든 결말을 미래의 다짐으로 바꾸지 않는다. voices는 말투만 참고하는 자료다. voices의 제목이나 경험·성과·감정·동기는 사실 근거로 사용할 수 없다. 문체는 종결만 맞추는 것이 아니다. 예문에서 짧은 진술 뒤에 이유를 길게 설명하는 순서, 스스로 왜 그런지 묻는 방식, A가 아니라 B라는 대조, 기술 용어를 일상적인 생각과 잇는 방식을 관찰해 이번 사실이 허용하는 곳에 구체적으로 적용한다. 문항에 맞지 않는 비유나 감정은 넣지 않는다. 고유 사건과 긴 문장을 복사하지 않되 범용적인 연결 표현과 문장 구조는 활용한다. 사용자 초안의 살아 있는 표현을 평범한 요약으로 바꾸지 않는다.
